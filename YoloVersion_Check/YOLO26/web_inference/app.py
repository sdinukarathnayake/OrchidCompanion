"""Browser inference for YOLO26 best.pt — loads weights only; never deletes trained_models/best.pt."""
...
YOLO26_ROOT = APP_DIR.parent

DEFAULT_MODEL_CANDIDATES = [
    YOLO26_ROOT / "trained_models" / "best.pt",
    YOLO26_ROOT / "yolo26_runs" / "yolo26s_512_50epochs" / "weights" / "best.pt",
    YOLO26_ROOT / "yolo26_runs" / "yolo26s_512_100epochs" / "weights" / "best.pt",
    YOLO26_ROOT / "yolo26s.pt",
    APP_DIR / "best.pt",
    YOLO26_ROOT / "best.pt",
]
...
def _resolve_model_path() -> Path:
    ...
    for candidate in YOLO26_ROOT.rglob("best.pt"):
        if candidate.is_file():
            return candidate

    raise FileNotFoundError(
        "No weights found. Run train_run_yolo26_pc.py (trained_models/best.pt), place yolo26s.pt in the YOLO26 "
        "folder, copy best.pt into web_inference/, or set MODEL_PATH to a .pt file."
    )
...
def _aggregate_by_class(detections: list[dict]) -> list[dict]:
    ...
    for det in detections:
        ...
        g = grouped.setdefault(label, {"label": label, "max_conf": conf, "count": 0})
        g["count"] += 1
        g["max_conf"] = max(g["max_conf"], conf)
    rows = [
        {"label": lab, "confidence": round(v["max_conf"], 4), "count": v["count"]}
        for lab, v in grouped.items()
    ]
    rows.sort(key=lambda x: (-x["count"], -x["confidence"]))
    return rows
